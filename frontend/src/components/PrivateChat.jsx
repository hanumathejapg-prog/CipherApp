...
const PrivateChat = ({ selectedUser, messages, onSend, currentId, onMessageVisible }) => {
  ...
  return (
    <div className="chat-panel">
      <h3>{selectedUser ? selectedUser.name : 'Select a user for private chat'}</h3>
      <div className="messages">
        {messages.map((m, idx) => (
          <React.Fragment key={`${m.id || 'd'}-${idx}`}>
      ...
      <form onSubmit={submit} className="composer">
        ...
        <textarea
          ...
          placeholder={selectedUser ? 'Type a private message (Shift+Enter for new line)' : 'Select a user'}
          disabled={!selectedUser}
          rows="3"
        />
        <div className="composer-buttons">
          <button type="button" className="gif-btn" onClick={() => setShowGifPicker(true)} disabled={!selectedUser} title="Send GIF">
            GIF
          </button>
